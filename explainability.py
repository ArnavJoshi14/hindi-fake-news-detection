...
# FIX: flatten shap_values correctly before returning
...
# FIX: compute directly from coef_ instead of LinearExplainer
# which returns zeros for single-sample background
...
def full_explain(text, pipeline, lime_num_features=15, verbose=True):
    ...
    lime_exp, top_words = explain_tfidf_lime(text_lower, pipeline.tfidf_vectorizer, pipeline.tfidf_clf, lime_num_features)
    ...
    explanation = {
        "prediction": result,
        "lime":  {"exp": lime_exp, "top_words": top_words},
...
def _print_explanation(exp):
    ...
    for word, weight in exp["lime"]["top_words"][:10]:
...
# 6. PLOT HELPERS
# ══════════════════════════════════════════════════════════════
def plot_lime(explanation, save_path=None):
    import matplotlib.pyplot as plt
    words  = [w for w, _ in explanation["lime"]["top_words"]]
    scores = [s for _, s in explanation["lime"]["top_words"]]
